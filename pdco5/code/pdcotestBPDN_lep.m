function [x_pdco,OV]=pdcotestBPDN_lep( A,b,lambda )
...
%   end
  K = A;
  [m,n]=size(A);
  [A,b,bl,bu,c,d1,d2] = toydata( A,b,lambda ); % Private function below
  ...
  Xt = Xtotal(1:n,:)-Xtotal(n+1:end,:);
  for it =1:size(Xtotal,2)
     OV(it) = 0.5*norm(K*Xt(:,it)-b).^2+lambda*norm(Xt(:,it),1);
  end
% Rebuild the solution
  x_pdco  = x(1:n) - x(n+1:end);
...
function [Aop,b,bl,bu,c,d1,d2] = toydata(  A,b,lambda )
