...
%               filename: original file name and FilePath
...
if nargin==2
    if DataType==0 %raw data
        ...
        ProcPathRoot=[FilePath 'pdata' OSslash '1'];
...
if FilePath
    ...
    elseif exist([FilePath OSslash 'acqus'], 'file') == 2
        ...
        error('Can not find the acqus file')
    end    
    ...
    if DataDim>=1
        ...
        if exist(Procpath, 'file') == 2
            ...
            disp(['cannot open file: ' Procpath])
    ...
    if DataDim>=2
        ...
        if exist(Procpath, 'file') == 2
            ...
            disp(['cannot open file: ' Procpath])
            disp('No processing paramaters for the second dimension');
            brukerdata.proc2s=[];
        end
    end
    ...
    if DataDim>=3
        ...
        if exist(Procpath, 'file') == 2
            ...
            disp(['cannot open file: ' Procpath])
            disp('No processing paramaters for the third dimension');
            brukerdata.proc3s=[];
        end
    end    
...
%          tnp=brukerdata.np
        ...
        brukerdata.at=brukerdata.at/(brukerdata.SI/brukerdata.np);
    ...
    %Importing lists
    %Assuming it is either a diffusion or variable delay experiment if
    %arrayed
    
    %setting  diffusion parameters
    gzlvlpath=[FilePath 'difflist'];
    ...
    %importing vdlist or vclist if existing  
    vdlistpath=[FilePath 'vdlist'];
    vclistpath=[FilePath 'vclist']; 
    fileid_vclist=fopen(vclistpath,'rt');
    fileid_vdlist=fopen(vdlistpath,'rt');
    
    if (fileid_vdlist~=-1)
        disp('reading vdlist for delay values (in T1/T2 experiment)')
        for k=1:str2num(brukerdata.acqu2s.TD)
            parmline=fgetl(fileid_vdlist);
            brukerdata.vdlist(k)=str2double(parmline);
            if parmline==-1;
                disp('Warning! -number of delay values in vdlist does not correspond to TD ')
                break;
            end;
        end
        fclose(fileid_vdlist);        
    end
    if (fileid_vclist~=-1)
        %disp('No vdlist so reading vclist for delay values')
        disp('reading vclist for loop counter (in T2 experiment)')
        for k=1:str2num(brukerdata.acqu2s.TD)
            parmline=fgetl(fileid_vclist);
            brukerdata.vclist(k)=str2double(parmline);
            if parmline==-1;
                disp('Warning! -number of delay values in vclist does not correspond to TD ')
                break;
            end;
        end
          fclose(fileid_vclist);
    end
    
    if isfield(brukerdata,'vclist') && isfield(brukerdata,'vdlist')
        disp('using vdlist directly for delay values, overriding vclist, but saving it for manual conversion')
        brukerdata.vc_constant=(2*brukerdata.acqus.D(19) + brukerdata.acqus.P(1)*1e-6);
    elseif isfield(brukerdata,'vdlist')
        disp('using vdlist directly for delay values')
    elseif isfield(brukerdata,'vclist')
        disp('Assuming vclist is for a T2 experiment. Converting to vdlist by vclist*(2*d20 + p2)')       
        brukerdata.vc_constant=(2*brukerdata.acqus.D(19) + brukerdata.acqus.P(1)*1e-6);
        brukerdata.vdlist=brukerdata.vclist*brukerdata.vc_constant;
    else
        disp('no vdlist or vclist file; cannot read in delay values')
        disp('If this is not a variable delay (e.g. T1/T2 measurments) data file, it is completely normal')
    end
    ...
    if isfield(brukerdata,'Gzlvl')
        ...
        brukerdata.ngrad=length(brukerdata.Gzlvl);
        disp(['DOSY pulse programme is: ' brukerdata.pulprog])
        ...
        brukerdata.ngrad=1;
        if DataDim==3
            disp('This seems to be 3D not diffusion data, assuming it is for PARAFAC so I make one dimension a "fake" diffusion dimension with dummy data')
            brukerdata.Gzlvl=zeros(1,str2double(brukerdata.acqu2s.TD));
            brukerdata.ngrad=length(brukerdata.Gzlvl);
            brukerdata.ni=str2double(brukerdata.acqu3s.TD);
...
%                 plot(real((FID)))
%                 
                %brukerdata.np=round(brukerdata.np/2);
...
%                 plot(real(fftshift(fft((FID)))))
        ...
        elseif DataDim==2
            ...
            if exist(ProcFIDpath, 'file') == 2
              ...
              %  ProcFIDpath
                ...
                %realfid=fread(fileid_ProcFIDreal,brukerdata.np*2*brukerdata.arraydim,byte_size);
        ...
        if m<brukerdata.np
            disp('Actual points in FID and number of points resported (TD) are different. This can hapen after the use of the "convdta" macro. I will use the actual number and continue import')
            brukerdata.np=m;
            FID=zeros(brukerdata.np,brukerdata.arraydim);
        end
